...
export async function GET() {
  try {
    ...
    const userWeight = loggedInUser.weight ? Number(loggedInUser.weight) : null;
    ...
    if (lastWorkout) {
      const workoutWeight = lastWorkout.userWeight ? Number(lastWorkout.userWeight) : null;
      ...
      workout.exerciseWorkouts.forEach(exercise => {
        ...
        sets.forEach(set => {
          ...
          if (exerciseLogType === 4) { // BodyWeight
            ...
            const bodyWeight = workoutWeight || 0;
            const additionalWeight = setWeight;
            weekWeightTotal += (bodyWeight + additionalWeight) * reps;
          } else if (exerciseLogType === 6) { // BodyWeightWithAssistance
            // For assisted bodyweight exercises, subtract assistance weight from stored workout weight
            const bodyWeight = workoutWeight || 0;
            const assistanceWeight = setWeight;
            weekWeightTotal += (bodyWeight - assistanceWeight) * reps;
          } else {
            // For regular weight exercises
      ...
      workout.exerciseWorkouts.forEach(exercise => {
        ...
        sets.forEach(set => {
          ...
          if (exerciseLogType === 4) { // BodyWeight
            ...
            const bodyWeight = workoutWeight || 0;
            const additionalWeight = setWeight;
            monthWeightTotal += (bodyWeight + additionalWeight) * reps;
          } else if (exerciseLogType === 6) { // BodyWeightWithAssistance
            // For assisted bodyweight exercises, subtract assistance weight from stored workout weight
            const bodyWeight = workoutWeight || 0;
            const assistanceWeight = setWeight;
            monthWeightTotal += (bodyWeight - assistanceWeight) * reps;
          } else {
            // For regular weight exercises
      ...
      workout.exerciseWorkouts.forEach(exercise => {
        ...
        sets.forEach(set => {
          ...
          if (exerciseLogType === 4) { // BodyWeight
            ...
            const bodyWeight = workoutWeight || 0;
            const additionalWeight = setWeight;
            yearWeightTotal += (bodyWeight + additionalWeight) * reps;
          } else if (exerciseLogType === 6) { // BodyWeightWithAssistance
            // For assisted bodyweight exercises, subtract assistance weight from stored workout weight
            const bodyWeight = workoutWeight || 0;
            const assistanceWeight = setWeight;
            yearWeightTotal += (bodyWeight - assistanceWeight) * reps;
          } else {
            // For regular weight exercises
        ...
        workout.exerciseWorkouts.forEach(exercise => {
          ...
          sets.forEach(set => {
            ...
            if (exerciseLogType === 4) { // BodyWeight
              ...
              const bodyWeight = workoutWeight || 0;
              const additionalWeight = setWeight;
              dayWeightTotal += (bodyWeight + additionalWeight) * reps;
            } else if (exerciseLogType === 6) { // BodyWeightWithAssistance
              // For assisted bodyweight exercises, subtract assistance weight from stored workout weight
              const bodyWeight = workoutWeight || 0;
              const assistanceWeight = setWeight;
              dayWeightTotal += (bodyWeight - assistanceWeight) * reps;
            } else {
              // For regular weight exercises
