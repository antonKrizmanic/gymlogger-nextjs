...
export function Combobox<T>({
  ...
  const defaultFilterFunction = (item: T, query: string) => {
    return getItemDisplay(item).toLowerCase().replace(/\s+/g, "").includes(query.toLowerCase().replace(/\s+/g, ""))
  }
    ...
    <div className={cn("relative", className)}>
      ...
      <Popover open={open} onOpenChange={setOpen}>
        ...
        <PopoverContent className="w-full p-0" align="start">
          <Command>
            <CommandInput placeholder={placeholder} onValueChange={setQuery} className="h-9" />
            <CommandList>
              {isLoading ? (
                  ...
                  {filteredItems.map((item) => (
                    ...
                    >
                      {renderItem ? (
                        ...
                        <>
                          <Check
                            className={cn("mr-2 h-4 w-4", value === getItemValue(item) ? "opacity-100" : "opacity-0")}
                          />
                          {getItemDisplay(item)}
                        </>
                      )}
