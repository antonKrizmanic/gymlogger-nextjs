...
export function ExerciseSets({ exercise }: ExerciseSetsProps) {
  ...
  const columns: Column[] = (() => {
    ...
    if (exerciseType === ExerciseLogType.BodyWeightWithAdditionalWeight) {
      cols.push({ key: "reps", label: "Reps", className: "flex-1" })
      cols.push({ key: "additionalWeight", label: "Additional", className: "flex-1" })
    }
    if (exerciseType === ExerciseLogType.BodyWeightWithAssistance) {
      cols.push({ key: "reps", label: "Reps", className: "flex-1" })
      cols.push({ key: "assistanceWeight", label: "Assistance", className: "flex-1" })
    }
  ...
  const renderCell = (key: string, set: any) => {
    switch (key) {
      case "index":
        return <span className="font-bold text-primary">{(set.index ?? 0) + 1}</span>
      case "reps":
        return <span className="font-medium">{set.reps ?? "-"}</span>
      case "weight":
        return <span className="font-medium">{set.weight ?? "-"} {set.weight != null ? "kg" : ""}</span>
      case "additionalWeight":
        return <span className="font-medium">{set.weight ?? "-"} {set.weight != null ? "kg" : ""}</span>
      case "assistanceWeight":
        return <span className="font-medium">{set.weight ?? "-"} {set.weight != null ? "kg" : ""}</span>
      case "time":
        return <span className="font-medium">{set.time != null ? `${set.time}s` : "-"}</span>
      case "note":
        return <span className="text-sm text-muted-foreground">{set.note || "-"}</span>
      default:
        return null
    }
  }
      ...
      <div className="hidden md:block">
        <div className="rounded-lg border bg-card">
          ...
          <div className="flex items-center gap-3 px-4 py-2 bg-muted/30 rounded-t-lg">
            {columns.map((col) => (
              <div key={col.key} className={`text-xs uppercase tracking-wide text-muted-foreground font-semibold ${col.className || "flex-1"}`}>
                {col.label}
              </div>
            ))}
          </div>
          ...
          <div className="divide-y">
            {exercise.sets.map((set) => (
              <div key={set.id ?? set.index} className="flex items-start gap-3 px-4 py-3 hover:bg-muted/20">
                {columns.map((col) => (
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex flex-wrap gap-2 text-xs text-muted-foreground">
                {exerciseType === ExerciseLogType.WeightAndReps && (
                  <>
                    <span className="px-2 py-1 rounded bg-muted/50">Reps: {set.reps ?? "-"}</span>
                    <span className="px-2 py-1 rounded bg-muted/50">Weight: {set.weight ?? "-"}{set.weight != null ? " kg" : ""}</span>
                  </>
                )}
                ...
                {exerciseType === ExerciseLogType.BodyWeightWithAdditionalWeight && (
                  <>
                    <span className="px-2 py-1 rounded bg-muted/50">Reps: {set.reps ?? "-"}</span>
                    <span className="px-2 py-1 rounded bg-muted/50">Additional: {set.weight ?? "-"}{set.weight != null ? " kg" : ""}</span>
                  </>
                )}
                {exerciseType === ExerciseLogType.BodyWeightWithAssistance && (
                  <>
                    <span className="px-2 py-1 rounded bg-muted/50">Reps: {set.reps ?? "-"}</span>
                    <span className="px-2 py-1 rounded bg-muted/50">Assistance: {set.weight ?? "-"}{set.weight != null ? " kg" : ""}</span>
                  </>
                )}
                ...
                {exerciseType === ExerciseLogType.TimeOnly && (
                  <span className="px-2 py-1 rounded bg-muted/50">Time: {set.time != null ? `${set.time}s` : "-"}</span>
                )}
              </div>
            </div>
            {set.note && (
